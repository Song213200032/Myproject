...
writer =SummaryWriter('./EdgeNet_mixed66')
...
if __name__=='__main__':
    ...
    for epoch in range(nepoch):
        ...
        with SummaryWriter('./EdgeNet_mixed66') as w:
            w.add_graph(modell, (ppp))
        
        writer.close()

        torch.save(classifier.state_dict(), '%s/seg_Model_EdgeNet_66_%d.pth' % (outf, epoch))

    x,y = Cor_Pred(tensor21,tensor22,nepoch)
    ddx = pd.DataFrame(x)
    ddy = pd.DataFrame(y)
    ddx.to_csv('./Edgenet_66_cor_pred.csv') 
    ddy.to_csv('./Edgenet_66_wor_pred.csv')
