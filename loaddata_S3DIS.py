...
class ShapeNetDataset(data.Dataset):
    def __init__(self,
                 ...
                 npoints=4096,
                 split='train'):
        self.root = root #'E:\DATA1\S3DIS\TrainDATA'
        self.npoints=npoints
        self.fileList=[]
        renfiledirList=os.listdir(root)
        #renfiledirList2=os.listdir('E:\DATA1\S3DIS\TestDATA')
        if(split=='train'):
            for renfiledir in range(int(len(renfiledirList))):
                for filedir in os.listdir(os.path.join(root,renfiledirList[renfiledir])):
                    self.fileList.append(os.path.join(root,renfiledirList[renfiledir],filedir))
        if(split=="test"):
             for renfiledir in range(int(len(renfiledirList))):
                for filedir in os.listdir(os.path.join(root,renfiledirList[renfiledir])):
                    self.fileList.append(os.path.join(root,renfiledirList[renfiledir],filedir))
    ...
    def __getitem__(self, index):
        ...
        point_set = mat['a'][:,0:9].astype(np.float32)
        seg = mat['a'][:,9].astype(np.int64)
