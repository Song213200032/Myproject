...
from Model_SetTransformer import *
...
writer =SummaryWriter('./SetTransformer')
...
classifier = SetTransformer()

if Model_SetTransformer != '':
    classifier.load_state_dict(torch.load(Model_SetTransformer))
...
if __name__=='__main__':
    ...
    for epoch in range(nepoch):
        ...
        torch.save(classifier.state_dict(), '%s/seg_Model_SetTransformer_%d.pth' % (outf, epoch))

    x,y = Cor_Pred(tensor21,tensor22,nepoch)
    ddx = pd.DataFrame(x)
    ddy = pd.DataFrame(y)
    ddx.to_csv('./SetTransformer_cor_pred.csv') 
    ddy.to_csv('./SetTransformer_wor_pred.csv')
 ...
 ##模型训练完之后，最后分割一遍，测试效果
    ...
    dda = pd.DataFrame(a)
    ddb = pd.DataFrame(b)
    dda.to_csv('./SetTransformer_d_cor_pred.csv')
    ddb.to_csv('./SetTransformer_d_wor_pred.csv')
    ddx.to_csv('./SetTransformer_test_cor_pred.csv')
    ddy.to_csv('./SetTransformer_test_wor_pred.csv')
