...
from Model_Pointnet_2 import *
from loaddata2 import ShapeNetDataset
...
writer =SummaryWriter('./Pointnet_2')
...
if Model_Pointnet_2 != '':
    classifier.load_state_dict(torch.load(Model_Pointnet_2))
...
if __name__=='__main__':
    ...
    for epoch in range(nepoch):
        ...
        torch.save(classifier.state_dict(), '%s/seg_Model_Pointnet_2_%d.pth' % (outf, epoch))

    x,y = Cor_Pred(tensor21,tensor22)
    ddx = pd.DataFrame(x)
    ddy = pd.DataFrame(y)
    ddx.to_csv('./Pointnet_2_cor_pred.csv')
    ddy.to_csv('./Pointnet_2_wor_pred.csv')
