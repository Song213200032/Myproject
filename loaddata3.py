...
class ShapeNetDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        point_set1 = mat['xyzivb_ti'][:,0:5].astype(np.float32)
        #point_set2 = mat['xyzivb_ti'][:,7].astype(np.float32)
        #point_set2 = np.expand_dims(point_set2,axis=1)
        #point_set = np.concatenate((point_set1,point_set2),axis=1)
        point_set = point_set1
        seg = mat['xyzivb_ti'][:,7].astype(np.int64)#三分类
        seg_6 = mat['xyzivb_ti'][:,5].astype(np.int64)#六分类
