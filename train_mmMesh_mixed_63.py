...
writer =SummaryWriter('./mmMesh_mixed63')
...
if __name__=='__main__':
    ...
    for epoch in range(nepoch):
        ...
        torch.save(classifier.state_dict(), '%s/seg_Model_mmMesh_63_%d.pth' % (outf, epoch))

    x,y = Cor_Pred(tensor21,tensor22,nepoch)
    ddx = pd.DataFrame(x)
    ddy = pd.DataFrame(y)
    ddx.to_csv('./mmMesh_63_cor_pred.csv') 
    ddy.to_csv('./mmMesh_63_wor_pred.csv')
