...
if model4 != '':
    classifier.load_state_dict(torch.load(model4))
...
h0=torch.zeros((2, 20, 24), dtype=torch.float32, device='cuda:0')#设置tensor的存放设备为0号cuda##############原来是288，修改为24
c0=torch.zeros((2, 20, 24), dtype=torch.float32, device='cuda:0')#设置tensor的存放设备为0号cuda##############原来是288，修改为24
...
if __name__=='__main__':
    for epoch in range(nepoch):
        ...
        for i, data in enumerate(dataloader, 0): #enumerate函数先返回下标i，后返回元素值data
            h=h0 #h0=[2,20,24]
            c=c0 #c0=[2,20,24]
            points,target=data #data中包含两个变量，分别是point_set 以及seg(标签数据)#points=[20 batch ,24 num_points ,5 num_dims]; target=[20 batch , 24 num_points];
            ...
            points = points.transpose(2, 1)#points=[20 batch ,5 num_dims ,24 num_points];
            optimizer.zero_grad()
            classifier.train()
            _ , pred,att, h, c = classifier(points,h,c)
        ...
        modell.train()
        dummy_input_point = torch.rand(20,5,12)
        dummy_input_h = torch.rand(2,20,24)
        dummy_input_c = torch.rand(2,20,24)
        with SummaryWriter('./Net') as w:
            w.add_graph(modell, (dummy_input_point,dummy_input_h,dummy_input_c))
        
        writer.close()

        torch.save(classifier.state_dict(), '%s/seg_model4_%d.pth' % (outf, epoch))
