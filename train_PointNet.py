...
if __name__=='__main__':
    ...
    for epoch in range(nepoch):
        ...
        torch.save(classifier.state_dict(), '%s/seg_Model_Pointnet_%d.pth' % (outf, epoch))

    x,y = Cor_Pred(tensor21,tensor22,nepoch)
    ddx = pd.DataFrame(x)
    ddy = pd.DataFrame(y)
    ddx.to_csv('./Pointnet_cor_pred.csv')
    ddy.to_csv('./Pointnet_wor_pred.csv')
    ...
    dda = pd.DataFrame(a)
    ddb = pd.DataFrame(b)
    dda.to_csv('./Pointnet_d_cor_pred.csv')
    ddb.to_csv('./Pointnet_d_wor_pred.csv')
    ddx.to_csv('./Pointnet_test_cor_pred.csv')
    ddy.to_csv('./Pointnet_test_wor_pred.csv')
