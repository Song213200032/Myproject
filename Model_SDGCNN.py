...
class STNkd2(nn.Module): #pointnet的仿射变换(变换k维(k=5))
...
class PointNetfeat2(nn.Module): #pointnet提取一帧中的全局特征，最后的最大池化变为Attention
    def __init__(self, global_feat = False, feature_transform = True):
        super(PointNetfeat2, self).__init__()
        self.k=5
        self.mylstm = nn.LSTM(input_size=3, hidden_size=3, num_layers=2, batch_first=True, dropout=0.1, bidirectional=False)
        self.stn = STNkd2(8)
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(8*2, 64, kernel_size=1, bias=False),
                                   self.bn1,
        ...
        if self.feature_transform:
            self.fstn = STNkd2(k=64)

    def forward(self, x):
        batch = x.shape[0]
        x_3 = x.cpu().numpy()
        x_3 = x_3[:,0:3,:]
        x_3 = torch.tensor(x_3).cuda()
        h0 = torch.zeros((2, batch, 3), dtype=torch.float32, device='cuda:0')
        c0 = torch.zeros((2, batch, 3), dtype=torch.float32, device='cuda:0')
        x1_3, (hn, cn) = self.mylstm(x_3.transpose(2,1), (h0, c0))
        x=torch.cat((x,x1_3.transpose(2,1)),1)
...
class GlobalTiRNN2(nn.Module):
    def __init__(self, points=12, k = 6, feature_transform=False):
        super(GlobalTiRNN2, self).__init__()
        self.points=points
        self.k=k
        self.feature_transform=feature_transform
...
class GlobalTiModule2(nn.Module):
    def __init__(self, points=12, k = 6, global_feat = False, feature_transform = True):
        super(GlobalTiModule2, self).__init__()
        self.points=points
        self.pointnet=PointNetfeat2(global_feat, feature_transform)#提取一帧中的特征
        self.grnn=GlobalTiRNN2(points, k, feature_transform)
