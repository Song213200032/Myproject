...
from loaddata3 import ShapeNetDataset
...
writer =SummaryWriter('./PointNet_mixed63')
...
classifier2 = GlobalTiModule(npoints, 3, feature_transform=feature_transform)

if Model_Pointnet != '':
    classifier.load_state_dict(torch.load(Model_Pointnet))
...
if __name__=='__main__':
    ...
    for epoch in range(nepoch):
        ...
        torch.save(classifier.state_dict(), '%s/seg_Model_Pointnet_63_%d.pth' % (outf, epoch))

    x,y = Cor_Pred(tensor21,tensor22,nepoch)
    ddx = pd.DataFrame(x)
    ddy = pd.DataFrame(y)
    ddx.to_csv('./Pointnet_63_cor_pred.csv')
    ddy.to_csv('./Pointnet_63_wor_pred.csv')
