...
if Model_Pointnet_3 != '':
    classifier.load_state_dict(torch.load(Model_Pointnet_3))
...
if __name__=='__main__':
    ...
    for epoch in range(nepoch):
        ...
        torch.save(classifier.state_dict(), '%s/seg_Model_Pointnet_3_%d.pth' % (outf, epoch))

    x,y = Cor_Pred(tensor21,tensor22)
    ddx = pd.DataFrame(x)
    ddy = pd.DataFrame(y)
    ddx.to_csv('./Pointnet_3_cor_pred.csv')
    ddy.to_csv('./Pointnet_3_wor_pred.csv')
