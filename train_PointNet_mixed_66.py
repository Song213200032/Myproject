...
writer =SummaryWriter('./PointNet_mixed66')
...
classifier2 = GlobalTiModule(npoints, num_classes, feature_transform=feature_transform)
...
if __name__=='__main__':
    ...
    for epoch in range(nepoch):
        ...
        torch.save(classifier.state_dict(), '%s/seg_Model_Pointnet_66_%d.pth' % (outf, epoch))

    x,y = Cor_Pred(tensor21,tensor22,nepoch)
    ddx = pd.DataFrame(x)
    ddy = pd.DataFrame(y)
    ddx.to_csv('./Pointnet_66_cor_pred.csv')
    ddy.to_csv('./Pointnet_66_wor_pred.csv')
