...
writer =SummaryWriter('./EdgeNet_mixed63')
...
if __name__=='__main__':
    ...
    for epoch in range(nepoch):
        ...
        for i, data in enumerate(dataloader, 0): #enumerate函数先返回下标i，后返回元素值data
            ...
            predd = predd.view(-1, 3) #重构张量的维度（x，6）
        ...
        with SummaryWriter('./EdgeNet_mixed63') as w:
            w.add_graph(modell, (ppp))
        
        writer.close()

        torch.save(classifier.state_dict(), '%s/seg_Model_EdgeNet_63_%d.pth' % (outf, epoch))

    x,y = Cor_Pred(tensor21,tensor22,nepoch)
    ddx = pd.DataFrame(x)
    ddy = pd.DataFrame(y)
    ddx.to_csv('./Edgenet_63_cor_pred.csv') 
    ddy.to_csv('./Edgenet_63_wor_pred.csv')
