...
classifier = GlobalTiModule2(npoints, num_classes, feature_transform=feature_transform)
classifier2 = GlobalTiModule2(npoints, num_classes, feature_transform=feature_transform)


if Model_SDGCNN != '':
    classifier.load_state_dict(torch.load(Model_SDGCNN))

optimizer = optim.Adam(classifier.parameters(), lr=0.01, betas=(0.9, 0.999))
scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.5)
optimizer2 = optim.Adam(classifier.parameters(), lr=0.01, betas=(0.9, 0.999))
...
if __name__=='__main__':
    ...
    for epoch in range(nepoch):
        ...
        torch.save(classifier.state_dict(), '%s/seg_Model_SDGCNN_%d.pth' % (outf, epoch))

    x,y = Cor_Pred(tensor21,tensor22,nepoch)
    ddx = pd.DataFrame(x)
    ddy = pd.DataFrame(y)
    ddx.to_csv('./SDGCNN_cor_pred.csv') 
    ddy.to_csv('./SDGCNN_wor_pred.csv')
